...
checksum(Sheet) ->
  lists:foldl(fun(Row, Acc) -> dist(Row) + Acc end, 0, Sheet).

dist(Row) -> dist(Row, hd(Row), hd(Row)).
dist([], M, N) when M < 0, N < 0 -> abs(M) - abs(N);
dist([], M, N) when M < 0, N >= 0 -> abs(M) + N;
dist([], M, N) when M >= 0, N >= 0 -> N - M;
dist([H|T], M, N) when H < M -> dist(T, H, N);
dist([H|T], M, N) when H > N -> dist(T, M, H);
dist([_|T], M, N) -> dist(T, M, N).
